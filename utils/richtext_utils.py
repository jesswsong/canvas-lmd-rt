...
def parse_json_lmd(json_str, hex_code):
    r"""
    ...
    for span in json_str['ops']:
        ...
        # parsing of the color prompts in josn
        if 'color' in span['attributes']:
            use_grad_guidance = True
            color_rgb, nearest_color = hex_to_rgb(hex_code, True)
            print(f"ORIGINAL COLOR: {span['attributes']['color']}") #hex code
            if prev_color_rgb == color_rgb:
                prev_text_prompt = color_text_prompts[-1]
                color_text_prompts[-1] = prev_text_prompt + \
                    ' ' + text_prompt
            else:
                color_rgbs.append(color_rgb) #list of objects & colors
...
def get_region_diffusion_input(
    model, 
    base_text_prompt, 
    style_text_prompts, 
    footnote_text_prompts,
...
def get_region_diffusion_modified(
    model, 
    base_text_prompt, 
    color_text_prompts, 
    color_names
