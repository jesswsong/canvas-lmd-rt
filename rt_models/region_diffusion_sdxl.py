                ...
                for i, t in enumerate(self.scheduler.timesteps):
                    ...
                    if use_guidance and t < text_format_dict['guidance_start_step']:
                        with torch.enable_grad():
                            ...
                            # Predict original image from nosiy iamge
                            ...
                            # transform the scaled latent representations back into image space
                            imgs = self.vae.decode(latents_inp.to(dtype=torch.float16)).sample
                            
                            # Normalize and Clamp Images: Normalizes the images to have values between 0 and 1, ensuring they fall within the valid range for image data.
                            imgs = (imgs / 2 + 0.5).clamp(0, 1) # clamp all elements of input into range [min, max]
                            
                            loss_total = 0.
                            
                            # Compute Color Loss: For each attention map and target RGB value pair, 
                            # the code calculates the average RGB values where the attention map is applied, 
                            # effectively focusing the loss calculation on specific regions of the image.
                            for attn_map, rgb_val in zip(text_format_dict['color_obj_atten'], text_format_dict['target_RGB']):
                                print(f'RGB VAL SHAPE: {rgb_val.shape}')
                                avg_rgb = (
